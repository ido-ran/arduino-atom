...
exports.doCompile = function(code,board,sketch, cb, publishEvent) {
    publishEvent = publishEvent || function() {};
    ...
    var sketchDir = path.join(OPTIONS.platform.getUserSketchesDir(), sketch);
    fs.writeFileSync(inoFile, code);

    publishEvent({ type:'compile', message:'writing to ' + inoFile });

    OPTIONS.platform.installIfNeeded(function(err) {
      if (err) {
        return cb(err);
      }
        OPTIONS.name = sketch;
        compile.compile(sketchpath,outpath,OPTIONS, publishEvent,
                sketchDir, function(err) {
                  if (err) return cb(err);
                  cb(null /* no error */,
                    {
                      hexFile: outpath +'/'+sketch+'.hex',
                      options: OPTIONS
                  });
                });
    }, function(per) {
